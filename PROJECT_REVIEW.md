...
sur Windows, macOS et Linux. Les risques prioritaires restent le packaging
Desktop, les documents/exportations très lourds, la robustesse des traitements
locaux et la taille de `App.tsx` / `App.css`.
...
| Fichiers | Sauvegarde / Save As | Partiel | export multipart, `SaveAsDialog.tsx`, tests save/export | Le Web télécharge ; le choix de destination natif définitif reste à faire | P1 release Desktop |
...
| Viewer | Gros PDF / mémoire | Fragile | seuils dans `App.tsx`, `multi-document.spec.ts`, `PDF-PERF-001` | PDF.js, IndexedDB et exports conservent encore des documents complets | P1 mesures et garde-fous |
...
| OCR | OCR local fra/eng/mixte et réouverture | Partiel | `OcrDialog.tsx`, `/ocr`, `tests/test_ocr.py`, E2E OCR | Qualité/durée dépendent des binaires et du scan ; packaging OS incomplet | P1 packaging et feedback |
...
| Desktop | Sidecar, health/restart et port dynamique en développement | Complet | `apps/desktop/src-tauri`, `DESKTOP.md`, tests du lanceur | Ce statut ne vaut pas validation d'installateur | Maintenir les checks Desktop |
| Desktop | Installateurs, association PDF, double-clic, signatures/notarisation, mises à jour | Partiel | limites explicites `DESKTOP.md`, `AGENTS.md` | Non validé pour publication générale | P1 release |
...
| Sécurité | Traitement local, validation formats, temporaires et limites conversion | Partiel | `CONVERSION.md`, validations backend et erreurs métier | Corpus hostile, annulation subprocess et nettoyage en échec doivent être qualifiés plus largement | P1 robustesse |
...
1. `README.md` affirme que les appearances des widgets gérés par la couche
   interactive ne sont pas rendues dans le canvas. Cette formulation est trop
   générale depuis `FORM-READONLY-RENDER-001` : les widgets **nativement
   readonly** conservent volontairement leur apparence canvas pour éviter la
   duplication visuelle.
2. Le README emploie encore les libellés historiques « Verrouiller l’édition »
   et « Verrouiller le formulaire ». L'UI actuelle les expose dans le popover
   du cadenas, avec une action PDF explicitement distinguée du verrou local.
3. `QA_BROWSER_REPORT.md` reste historique. `QA_AUTOMATION.md` décrit le
...
4. La CI backend exécute désormais réellement OCR et Ruff sur Ubuntu 24.04,
...
| P1 | Mesures structurées mémoire/export sur corpus 250+ pages et limites produit | Élevé / moyen | M | fixtures synthétiques, instrumentation navigateur |
| P1 | Robustesse entrées/subprocess : PDF anormal, timeout, annulation et nettoyage | Élevé / moyen | M | moteur Python, corpus sécurité |
...
1. Corriger les deux formulations README sur les readonly et le verrouillage
   afin que la documentation utilisateur reflète l'UI réellement livrée.
2. Ajouter un smoke test Tauri dès qu'un runner natif est disponible.
3. Publier une matrice de support par OS pour OCR/conversion/impression et
   l'associer au protocole d'installation.
4. Créer un corpus synthétique de PDF chiffrés/corrompus, de formulaires aux
   appearances atypiques et d'exports lourds.
5. Extraire progressivement les domaines de `App.tsx` sans mélanger refactor
